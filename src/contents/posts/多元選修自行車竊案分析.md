...
---
## 概述
這個專題在分析台北市自行車竊案公開數據，最初是用 Google Colab 透過檔案上傳處理數據，後改進為 Flask 網頁應用程式，結合台北市資料開放平台 API 實現即時數據查詢，並部署至 Vercel。
...
### 部署過程
1. **準備專案結構**：
   - 包含 `app.py`（主程式）、`templates/index.html`（前端模板）、`requirements.txt`（依賴清單）。
   - 新增 `vercel.json`：
     ```json
     {
       "version": 2,
...
4. **部署至 Vercel**：
   - 在 Vercel 匯入 GitHub 倉庫，選擇 Python 框架，自動建構並部署。
